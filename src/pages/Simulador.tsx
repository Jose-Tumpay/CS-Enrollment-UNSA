...
import {
  Info, Play, Trash2, ArrowRight, Lock, CheckCircle2,
  Calendar, AlertTriangle, Zap, RefreshCw, Ban, GraduationCap,
  ArrowDown, TriangleAlert,
} from 'lucide-react';
import { ACADEMIC_RULES } from '../config/academicRules';

export function Simulador() {
  const engine = useCurriculumEngine();
  const { profile, toggleCourseSimulated, clearSimulated, approveSimulated, setEnrollmentTarget } = useStudentStore();

  if (!engine || !profile) return null;

  const { enrollmentSummary, enrollmentTarget: target } = engine;

  const currentYear = new Date().getFullYear();
  const yearOptions = Array.from(
    { length: ACADEMIC_RULES.enrollmentYearsBefore + ACADEMIC_RULES.enrollmentYearsAhead + 1 },
  ...
  const handleSimulateApproval = () => {
    if (confirm(`¿Confirmas marcar estos ${profile.simulatedCourses.length} cursos como aprobados permanentemente?`)) {
      approveSimulated();
    }
  };
  ...
  const regularCourses = availableForSemester.filter(c => !c.isInjectedEquivalent);

  // Plan 2025 equivalents injected for Plan 2017 students (replacements for retired courses)
  const injectedEquivalents = availableForSemester.filter(c => c.isInjectedEquivalent);

  // Rescheduled courses (for the notice banner)
  const rescheduledCourses = availableForSemester.filter(c => c.isRescheduled);

  // Retired 2017 courses with NO official 2025 equivalent — student is stuck
  const noEquivCourses = engine.activePlan === '2017'
  ...
  // ── Credit bar ──────────────────────────────────────────────────────────────
  const { totalCredits: simCredits, isUnderMinimum, isOverMaximum, creditWarning, selectedCourses, willUnlock } = enrollmentSummary;
  const creditPercent = Math.min((simCredits / ACADEMIC_RULES.maxCreditsPerSemester) * 100, 100);
  ...
  const retired2017YearsLabel = engine.retiredPlan2017CareerYears.length > 0
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8">
      ...
      {/* ── Header ───────────────────────────────────────────────────────────── */}
      <header>
        <h1 className="text-3xl font-bold tracking-tight">Simulador de Matrícula</h1>
        <p className="text-muted-foreground mt-1">
          Solo se muestran los cursos realmente ofertados para el periodo seleccionado, según tu plan y el avance de las promociones.
        </p>
      </header>
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex-1 min-w-[160px]">
            <label className="block text-sm font-medium text-muted-foreground mb-1.5">Año académico</label>
            <select
              value={target.year}
          ...
          <div className="flex-1 min-w-[200px]">
            <label className="block text-sm font-medium text-muted-foreground mb-1.5">Semestre</label>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div className="bg-primary/5 border border-primary/20 rounded-lg px-4 py-2.5 text-sm font-medium text-primary flex items-center gap-2">
            ...
            <span>
              <strong>{target.year} — Sem. {target.semester === 1 ? 'A' : 'B'}</strong>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-amber-600 dark:text-amber-400 text-sm">
                  {rescheduledCourses.length} curso{rescheduledCourses.length > 1 ? 's' : ''} reprogramado{rescheduledCourses.length > 1 ? 's' : ''} para este semestre
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Normalmente se dictan en el otro semestre, pero están disponibles excepcionalmente por reprogramación oficial.
                </p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            {regularCourses.length === 0 ? (
              <div className="text-center p-10 border-2 border-dashed border-border rounded-xl text-muted-foreground">
                ...
                <p className="font-medium">No hay cursos propios disponibles para este semestre.</p>
          ...
          {engine.activePlan === '2017' && (injectedEquivalents.length > 0 || noEquivCourses.length > 0) && (
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Estos cursos ya no se ofertan en {target.year} porque la promoción 2024 (última del Sílabo 2017) los superó.
                {injectedEquivalents.length > 0 && ' Los que tienen equivalencia oficial del Sílabo 2025 pueden matricularse normalmente.'}
              </p>
                ...
                <div className="space-y-3 mb-6">
                  <AnimatePresence initial={false}>
                    {injectedEquivalents.map(course => {
                      const isSelected = profile.simulatedCourses.includes(course.code);
                      return (
                        <motion.div
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0" onClick={() => toggleCourseSimulated(course.code)}>
                              {/* Top row: code + badges + credits */}
                              <div className="flex justify-between items-start mb-0.5 gap-2">
                                <span className="font-mono text-xs text-muted-foreground font-semibold">{course.code}</span>
                              ...
                              <p className="text-xs text-muted-foreground mt-0.5">
                                Año {course.year} — Sem {course.semester === 1 ? 'A' : 'B'} · Sílabo 2025
                              </p>

                              {/* Equivalence mapping */}
                              {(course.replacesEquiv2017Names?.length ?? 0) > 0 && (
                                <div className="mt-2 pl-3 border-l-2 border-rose-500/30">
                                  ...
                                  {course.replacesEquiv2017Names!.map((name, i) => (
                                    <div key={i} className="flex items-center gap-1.5">
                                      ...
                                      <span className="text-[10px] text-muted-foreground italic">· Sílabo 2017</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                ...
                <div className="space-y-3">
                  {noEquivCourses.map(course => (
                    ...
                    >
                      ...
                      {/* Name and location */}
                      <h3 className="font-bold text-sm mb-0.5">{course.name}</h3>
                      <p className="text-xs text-muted-foreground">
                        Año {course.year} — Sem {course.semester === 1 ? 'A' : 'B'} · Sílabo 2017
                      </p>
                      ...
                      <div className="mt-3 bg-amber-500/8 border border-amber-500/20 rounded-lg px-3 py-2.5 text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                        Este curso ya no se oferta y <strong>no posee una equivalencia oficial en el Sílabo 2025</strong>. Para aprobarlo sería necesaria una reprogramación extraordinaria o una disposición especial de la Escuela Profesional.
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {lockedThisSemester.length > 0 && (
            <div>
              <h2 className="text-xl font-bold border-b border-border pb-2 mb-4 text-muted-foreground flex items-center gap-2">
                ...
                <span className="text-sm font-normal">({lockedThisSemester.length})</span>
              </h2>
              <div className="space-y-2 opacity-60">
                {lockedThisSemester.map(course => (
                  <div key={course.code} className="flex items-center gap-4 p-3 rounded-xl border border-border bg-card/50">
          ...
          {wrongSemesterAvailable.length > 0 && (
            <div className="bg-muted/30 border border-border rounded-xl p-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-muted-foreground mb-2">
                ...
                {wrongSemesterAvailable.length} cursos disponibles en Semestre {target.semester === 1 ? 'B' : 'A'} (no este periodo)
              </div>
              <div className="flex flex-wrap gap-1.5">
                {wrongSemesterAvailable.slice(0, 10).map(c => (
                  <span key={c.code} className="text-xs bg-background border border-border px-2 py-1 rounded font-mono opacity-70">
                    {c.code}
                  </span>
                ))}
                {wrongSemesterAvailable.length > 10 && (
                  <span className="text-xs text-muted-foreground px-2 py-1">+{wrongSemesterAvailable.length - 10} más</span>
                )}
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-1 sticky top-6 space-y-4">
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
            ...
            <div className="mb-5">
              <div className="flex justify-between items-baseline mb-2">
                ...
                <span className={`font-bold text-2xl ${isOverMaximum ? 'text-destructive' : isUnderMinimum ? 'text-amber-500' : 'text-primary'}`}>
                  {simCredits}
              ...
              <div className="w-full bg-muted rounded-full h-2.5 overflow-hidden relative">
                <div
                  ...
                  style={{ left: `${(ACADEMIC_RULES.minCreditsPerSemester / ACADEMIC_RULES.maxCreditsPerSemester) * 100}%` }}
                />
                <div
            ...
            <div className="flex justify-between items-center text-sm py-2 border-t border-border">
              ...
              <span className="font-bold text-lg">{profile.simulatedCourses.length}</span>
            </div>
            ...
            <div className="space-y-3 mt-5">
              ...
              >
                ...
                Aprobar {profile.simulatedCourses.length > 0 ? `(${profile.simulatedCourses.length})` : 'Cursos'}
              </button>
              ...
              <div className="mt-6 pt-5 border-t border-border">
                ...
                <div className="space-y-2">
                  {willUnlock.slice(0, 6).map(c => (
                    <div key={c.code} className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                      <ArrowRight className="w-3.5 h-3.5 shrink-0" />
                      <span className="text-xs truncate">{c.name}</span>
                    </div>
                  ))}
                  {willUnlock.length > 6 && (
                    <p className="text-xs text-muted-foreground">+{willUnlock.length - 6} más cursos</p>
                  )}
                </div>
              </div>
            )}
          ...
          {selectedCourses.length > 0 && (
            <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
              <h4 className="font-semibold text-sm mb-3 text-muted-foreground">Cursos en tu matrícula</h4>
              <div className="space-y-2">
                  ...
                  return (
                    <div key={c.code} className="flex items-start justify-between gap-2 text-sm">
                      <div className="flex flex-col min-w-0">
                        ...
                        {isEquiv && (c.replacesEquiv2017Names?.length ?? 0) > 0 && (
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                        <span className="text-xs text-muted-foreground font-mono">{c.credits} cr</span>
                        <button
...
function CourseCardList({ courses, selectedCodes, onToggle }: CourseCardListProps) {
  return (
    <div className="space-y-3">
      <AnimatePresence initial={false}>
        {courses.map(course => {
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0" onClick={() => onToggle(course.code)}>
                <div className="flex justify-between items-start mb-0.5 gap-2">
                  <span className="font-mono text-xs text-muted-foreground font-semibold">{course.code}</span>
                  <div className="flex items-center gap-1.5 shrink-0">
                    {course.isRescheduled && (
