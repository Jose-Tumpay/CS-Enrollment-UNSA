...
import {
  Download, Upload, Printer, CheckCircle2, Layers,
  Trash2, ChevronDown, ChevronRight, BookCheck, Zap, RotateCcw
} from 'lucide-react';
...
export function Progreso() {
  ...
  const { profile, saveProfile, approveCourses, unapproveCourses, resetProfile } = useStudentStore();
  ...
  const handleImportJSON = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (json.entryYear && json.approvedCourses) {
          saveProfile(json);
  ...
  const coursesBySemester = (year: number, semester: 1 | 2) =>
    engine.courses.filter(c => c.year === year && c.semester === semester).map(c => c.code);

  const availableNow = engine.courses
    .filter(c => (c.missingPrerequisites?.length ?? 0) === 0 && c.status !== 'approved')
    .map(c => c.code);

  const confirm = (msg: string) => window.confirm(msg);

  const handleApproveYear = (year: number) => {
    if (!confirm(`¿Aprobar todos los cursos del Año ${year}? Esto actualizará tu historial.`)) return;
    approveCourses(coursesByYear(year));
  };

  const handleUnapproveYear = (year: number) => {
    if (!confirm(`¿Quitar aprobación de todos los cursos del Año ${year}?`)) return;
  ...
  const handleApproveAvailable = () => {
    ...
    if (!confirm(`¿Aprobar los ${availableNow.length} cursos actualmente disponibles?`)) return;
  ...
  const approvedBySemester = engine.progressBySemester
    ...
    .map(sem => {
      ...
      const courses = engine.courses.filter(
        c => c.year === year && c.semester === semesterPart && c.status === 'approved'
      );
      return { ...sem, courses };
    });
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 print:mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {engine.approvedCount} cursos aprobados · {engine.approvedCredits} créditos
          </p>
        </div>
        <div className="flex items-center gap-2 print:hidden flex-wrap">
          ...
          <label className="flex items-center gap-2 bg-background border border-border px-3 py-2 rounded-lg hover:bg-muted text-sm font-medium cursor-pointer transition-colors">
      ...
      <section className="bg-card border border-border rounded-2xl overflow-hidden print:hidden">
        <div className="p-4 border-b border-border bg-muted/20">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Marca o quita cursos en bloque para configurar tu historial rápidamente.
          </p>
        </div>
        ...
        <div className="divide-y divide-border">
          ...
          <div>
            ...
            >
              ...
              {openPanel === 'quick' ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
            </button>
            {openPanel === 'quick' && (
          ...
          <div>
            ...
            >
              ...
              {openPanel === 'year' ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
            </button>
            {openPanel === 'year' && (
              <div className="px-5 pb-4 space-y-2">
                {[1, 2, 3, 4, 5].map(year => {
                  const total = coursesByYear(year).length;
                  const approved = engine.courses.filter(c => c.year === year && c.status === 'approved').length;
                  return (
                    <div key={year} className="flex items-center justify-between gap-3 bg-muted/30 rounded-lg px-4 py-2.5">
                      <div>
                        <span className="font-semibold text-sm">Año {year}</span>
                        <span className="text-xs text-muted-foreground ml-2">{approved}/{total} cursos</span>
                      </div>
          ...
          <div>
            ...
            >
              ...
              {openPanel === 'semester' ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
            </button>
            {openPanel === 'semester' && (
              <div className="px-5 pb-4 space-y-2">
                {[1, 2, 3, 4, 5].flatMap(year =>
                  ([1, 2] as const).map(sem => {
                    ...
                    const approved = engine.courses.filter(
                      c => c.year === year && c.semester === sem && c.status === 'approved'
                    ).length;
                    return (
                      <div key={`${year}-${sem}`} className="flex items-center justify-between gap-3 bg-muted/30 rounded-lg px-4 py-2.5">
                        <div>
                          <span className="font-semibold text-sm">Año {year} — Sem {sem === 1 ? 'A' : 'B'}</span>
                          <span className="text-xs text-muted-foreground ml-2">{approved}/{codes.length} cursos</span>
                        </div>
                        <div className="flex gap-2 shrink-0">
                          <button
      ...
      <section className="bg-card border border-border rounded-2xl p-6 print:border-0 print:p-0">
        ...
        {approvedBySemester.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="font-medium">Aún no has marcado ningún curso como aprobado.</p>
            <p className="text-sm mt-1 opacity-70">Usa la Malla Curricular o las herramientas de arriba para comenzar.</p>
          </div>
        ) : (
                  ...
                  {sem.courses.map(course => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="font-mono text-xs text-muted-foreground font-semibold mb-0.5">{course.code}</div>
                        <div className="font-bold text-sm truncate" title={course.name}>{course.name}</div>
                      </div>
